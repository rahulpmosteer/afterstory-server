...
export class WillController {
  ...
  static async getWill(req: Request, res: Response) {
    try {
      ...
      const data = await WillService.getWill(consumerId);
  ...
  static async upsertWill(req: Request, res: Response) {
    try {
      ...
      return res.status(200).json({ success: true, data: will });
    } catch (error: any) {
  ...
  static async updateAsset(req: Request, res: Response) {
    try {
      ...
      const asset = await WillService.updateAsset(consumerId, assetId, validatedData);
  ...
  static async createBeneficiary(req: Request, res: Response) {
    try {
      ...
      const beneficiary = await WillService.createBeneficiary(consumerId, validatedData);
  ...
  static async deleteBeneficiary(req: Request, res: Response) {
    try {
      ...
      const result = await WillService.deleteBeneficiary(consumerId, beneficiaryId);
      return res.status(200).json({ success: true, ...result });
    } catch (error: any) {
      return res.status(500).json({ success: false, message: error.message });
    }
  ...
  static async saveDistribution(req: Request, res: Response) {
    try {
      ...
      const result = await WillService.saveDistribution(consumerId, validatedData);
      return res.status(200).json({ success: true, ...result });
    } catch (error: any) {
      if (error instanceof ZodError) {
        return res.status(400).json({
  ...
  static async markAsGenerated(req: Request, res: Response) {
    try {
      const consumerId = req.user!.id;
      const result = await WillService.markAsGenerated(consumerId);
      return res.status(200).json({ success: true, ...result });
    } catch (error: any) {
      return res.status(500).json({ success: false, message: error.message });
    }
